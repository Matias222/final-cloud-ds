...
ATHENA_TABLE = 'servicio-vuelos-compras-dev-compras'
...
def main():
    ...
    if query_results:
        ...
        for row in data: 
            ...
            dic={
            "precio_total":int(row[0]),
            "user_id":row[1],
            "fecha_compra":row[2],
            "id_vuelo":row[3],
            "id_compra":row[4],
            "cantidad_boletos":int(row[5])}
            
            temp.append(dic)
        
        funciones_bd.insert_multiple_rows_to_mysql("Compras",temp)
